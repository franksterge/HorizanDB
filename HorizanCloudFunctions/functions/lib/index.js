                ...
                for(let i = 0; i < ar2.length; i++){
                    if (!Object.keys(matches).includes(ar2[i])){
                        ...
                        if(SATScores[matches[i]] != 'NaN'){
                              sat_ratio = 1-Math.abs((uSAT - school_sat_score) / school_sat_score)
                        }
                        if(ACTScores[matches[i]] != 'NaN'){
                            act_ratio = 1-Math.abs((uACT / school_act_score) / school_act_score)
                        ...
                        ans_ob["act_ratio"] = act_ratio
                        ans_ob["sat_ratio"] = sat_ratio
                        ans_ob["overall_match"] = matchRate / 7
                        ans_ob["school"] = ar2[i]
                    
                        
                        matches[hash(ar2[i])]= ans_ob;
                    }
                }
            }
          }
