...
exports.handleUserData = functions.https.onRequest((request, response) => {

    let resp = JSON.parse(request.body)
    let ans = resp[0]
    let scoreArray = resp[1]
    // console.log(resp)
    // console.log(ans)
    ...
    //   console.log("computationABC start")
      ...
      let uni_raw = admin.database().ref("universities/").once("value", snap => {
      ...
      const numArray = [];
      let ans_perms = cartesian(ansArray)
                ...
                for(let i = 0; i < ar2.length; i++){
                    if (!Object.keys(matches).includes(ar2[i])){
                        ...
                        if(SATScores[matches[i]] != 'NaN'){
                              sat_ratio = Math.abs((uSAT / school_sat_score))
                        }
                        if(ACTScores[matches[i]] != 'NaN'){
                            act_ratio = Math.abs((uACT / school_act_score))
                        ...
                        let uni = unis[ar2[i].replace(".","_")]

                        uni["act_ratio"] = act_ratio
                        uni["sat_ratio"] = sat_ratio
                        uni["overall_match"] = (matchRate / 7) > 1 ? 1 : (matchRate / 7)
                        
                        matches[ar2[i].replace(".","_")]= uni;
