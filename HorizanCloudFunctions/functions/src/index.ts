...
exports.handleUserData = functions.https.onRequest((request, response) => {
    ...
    //   console.log("computation123 start")
            ...
            if(matchRate > 4){
                ...
                for(let i = 0; i < ar2.length; i++){
                    if (!Object.keys(matches).includes(ar2[i])){
                        ...
                        db.ref("universities/"+ ar2[i]).once("value", snap => {
                        var val1 = snap.val();
                        val1["act_ratio"] = act_ratio
                        val1["sat_ratio"] = sat_ratio
                        val1["overall_match"] = matchRate / 7
                        matches[ar2[i]]= val1;
                        }
                        )
                    }
